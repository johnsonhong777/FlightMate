				...
				<form action="${pageContext.request.contextPath}/aircraft"
					...
					<fieldset class="form-group mt-2">
						...
						<label for="nextMaintenanceDate" class="form-label">Next
							...
							name="nextMaintenanceDate" class="form-input" required><br>

						<label for="aircraftNotes" class="form-label">Aircraft
							Notes:</label>
						<textarea id="aircraftNotes" name="aircraftNotes"
							class="form-input"></textarea>
						<br> <label for="administratorId" class="form-label">Administrator:</label>
						<select id="administratorId" name="administratorId"
							class="form-input" required>
							<c:forEach var="admin" items="${administrators}">
								<option value="${admin.getUserId()}">${admin.getFirstName()}
									${admin.getLastName()}</option>
							</c:forEach>
						</select><br> <label for="airportId" class="form-label">Airport:</label>
						<select id="airportId" name="airportId" class="form-input"
							required>
		...
		<table class="dashboard-table w-full border-2 rounded mt-2">
			...
			<tbody>
				<c:forEach var="aircraft" items="${aircrafts}">
					...
					<form
						action="${pageContext.request.contextPath}/UpdateDeleteAircraftServlet"
						method="POST">
						<tr>
							<td>${aircraft.getAircraftId()} <input type="hidden"
								name="id" value="${aircraft.getAircraftId()}" />
							</td>
							<td><input type="text" class="form-input w-full"
								name="aircraft_model" value="${aircraft.getAircraftModel()}" />
							</td>
							<td><input type="date" class="form-input w-full"
								name="manufacture_date" value="${aircraft.getManufactureDate()}" />
							</td>
							<td><input type="date" class="form-input w-full"
								name="last_maintenance_date"
								value="${aircraft.getLastMaintenanceDate()}" /></td>
							<td><input type="date" class="form-input w-full"
								name="next_maintenance_date"
								value="${aircraft.getNextMaintenanceDate()}" /></td>
							<td><textarea class="form-input w-full"
									name="aircraft_notes">${aircraft.getAircraftNotes()}</textarea>
							</td>
							<td><label for="administratorId" class="form-label">Administrator:</label>
								<select id="administratorId" name="administratorId"
								class="form-input border-2 rounded" required>
									<c:forEach var="admin" items="${administrators}">
										<option value="${admin.getUserId()}">${admin.getFirstName()}
											${admin.getLastName()}</option>
									</c:forEach>
							</select></td>
							<td><label for="airportId" class="form-label">Airport:</label>
								<select id="airportId" name="airportId"
								class="form-input border-2 rounded" required>
									<c:forEach var="airport" items="${airports}">
										<option value="${airport.getAirportId()}">${airport.getAirportName()}</option>
									</c:forEach>
							...
							<c:if test="${user.getRole().equals(roles['ADMINISTRATOR'])}">
								<td><input type="submit" name="action" value="Update"
									class="btn w-full" /> <input type="submit" name="action"
									value="Delete" class="btn error w-full mt-2"
